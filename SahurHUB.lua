-- Proteccion autoexecute: esperar que el juego cargue completamente
if not game:IsLoaded() then game.Loaded:Wait() end
task.wait(0.5)

-- Esperar que el cliente este listo
...
-- Esperar ReplicatedStorage
local _RS = game:GetService("ReplicatedStorage")
local _ok = pcall(function()
    _RS:WaitForChild("Packages", 15)
    _RS:WaitForChild("Datas", 15)
    _RS:WaitForChild("Shared", 15)
    _RS:WaitForChild("Utils", 15)
end)
if not _ok then
    warn("[XI] ReplicatedStorage no cargó a tiempo, reintentando en 2s...")
    task.wait(2)
end
...
do
    ...
    for _attempt = 1, 5 do
        ...
        warn("[XI] Synchronizer intento " .. _attempt .. " fallido, reintentando...")
        task.wait(1)
    end
    if not _syncOk or not Sync then
        warn("[XI] No se pudo cargar Synchronizer, saltando patch...")
        Sync = {}
    end
    local patched = 0
    ...
    print("Sahur hub cargado, discord.gg/8HNEwrZUut")
end
...
-- Esperar PlayerGui con timeout para evitar crash en autoexecute
local PlayerGui
local _pgOk = false
for _i = 1, 10 do
    local ok, result = pcall(function()
...
if not _pgOk then
    PlayerGui = LocalPlayer:FindFirstChildOfClass("ScreenGui") and LocalPlayer.PlayerGui or LocalPlayer:WaitForChild("PlayerGui")
end

local Decrypted
Decrypted = setmetatable({}, {
    __index = function(S, ez)
        local ok_netty, Netty = pcall(function() return ReplicatedStorage.Packages.Net end)
        if not ok_netty or not Netty then return nil end
        local prefix, path
        if     ez:sub(1,3) == "RE/" then prefix = "RE/";  path = ez:sub(4)
        elseif ez:sub(1,3) == "RF/" then prefix = "RF/";  path = ez:sub(4)
        else return nil end
        local Remote
        local okC, children = pcall(function() return Netty:GetChildren() end)
...
local DefaultConfig = {
    ...
    LaserCapeAimbot = true,
    FOV = 70,
    SubspaceMineESP = true,
...
local function getControls()
    ...
    warn("[XI] getControls fallo, reintentando...")
    task.wait(1)
...
local function kickPlayer()
    LocalPlayer:Kick("\ndiscord.gg/8HNEwrZUut - xi loves you <3")
end

local _dropBrainrotCooldown = false
function doDropBrainrot()
    if _dropBrainrotCooldown then return end
    _dropBrainrotCooldown = true
    task.delay(1, function() _dropBrainrotCooldown = false end)
    ...
    lv.VectorVelocity = Vector3.new(0, 129, 0)
    lv.Parent = hrp
    task.wait(0.14)
    lv.VectorVelocity = Vector3.new(0, -55, 0)
    task.wait(0.05)
    lv:Destroy()
    att:Destroy()
end
...
if Config.FPSBoost then task.spawn(function() task.wait(2); pcall(setFPSBoost, true) end) end
...
if infiniteJumpEnabled then task.spawn(function() task.wait(1); pcall(setInfiniteJump, true) end) end
...
-- ============================================================
-- BASE TIMER ESP  (ported from NamelessHub)
-- Muestra el tiempo restante del purchase con menor timer
-- en cada plot del Workspace.Plots
-- ============================================================
local baseTimerESPEnabled = false
local baseTimerActiveConns = {}

local function _btesp_updateBillboard(mainPart, contentText, shouldShow)
    local existing = mainPart:FindFirstChild("XI_BaseTimerGui")
    if shouldShow then
        if not existing then
            local gui = Instance.new("BillboardGui")
            gui.Name = "XI_BaseTimerGui"
            ...
            if label then label.Text = contentText end
        end
    else
        if existing then existing:Destroy() end
    end
...
local function _btesp_findLowest(purchases)
...
local function _btesp_scanAndConnect()
    local plots = Workspace:FindFirstChild("Plots")
    if not plots then return end
    for _, plot in pairs(plots:GetChildren()) do
        local purchases = plot:FindFirstChild("Purchases")
        if purchases then
            local selected = _btesp_findLowest(purchases)
            for _, purchase in pairs(purchases:GetChildren()) do
                local main    = purchase:FindFirstChild("Main")
                local gui     = main and main:FindFirstChild("BillboardGui")
                local remTime = gui and gui:FindFirstChild("RemainingTime")
                ...
                if main and remTime and locked and remTime:IsA("TextLabel") and locked:IsA("GuiObject") then
                    ...
                    _btesp_updateBillboard(main, remTime.Text, isTarget)
                    local key = remTime:GetDebugId()
                    if isTarget and not baseTimerActiveConns[key] then
                        local function refresh()
                            local stillTarget = (_btesp_findLowest(purchases) or {}).remTime == remTime
                            _btesp_updateBillboard(main, remTime.Text, stillTarget and locked.Visible)
                        end
                        local c1 = remTime:GetPropertyChangedSignal("Text"):Connect(refresh)
                        local c2 = locked:GetPropertyChangedSignal("Visible"):Connect(refresh)
                        baseTimerActiveConns[key] = {c1, c2}
                    end
                end
            end
        end
...
local function enableBaseTimerESP()
    baseTimerESPEnabled = true
    Config.BaseTimerESP = true
    SaveConfig()
    task.spawn(function()
        while baseTimerESPEnabled do
            pcall(_btesp_scanAndConnect)
            task.wait(5)
        end
    end)
    ShowNotification("BASE TIMER ESP", "ENABLED")
end

local function disableBaseTimerESP()
    baseTimerESPEnabled = false
    Config.BaseTimerESP = false
    SaveConfig()
    -- Remove all timer billboards
    pcall(function()
        local plots = Workspace:FindFirstChild("Plots")
        if plots then
            for _, plot in pairs(plots:GetChildren()) do
                ...
                if purchases then
                    for _, purchase in pairs(purchases:GetChildren()) do
                        local main = purchase:FindFirstChild("Main")
                        if main then
                            local g = main:FindFirstChild("XI_BaseTimerGui")
    ...
    for _, conns in pairs(baseTimerActiveConns) do
        for _, c in ipairs(conns) do pcall(function() c:Disconnect() end) end
    end
    baseTimerActiveConns = {}
    ShowNotification("BASE TIMER ESP", "DISABLED")
end
...
-- ============================================================
-- ANTI RAGDOLL (version 44S Duel)
-- ============================================================
local _antiRagdollConns = {}

local function startAntiRagdollPrime()
    for _, c in pairs(_antiRagdollConns) do pcall(function() c:Disconnect() end) end
    _antiRagdollConns = {}
    ...
    local function setupChar(char)
        ...
        local c1 = RunService.Heartbeat:Connect(function()
            if not Config.AntiRagdollPrime then return end
            local isTping = State and State.isTpMoving
            ...
            if humState == Enum.HumanoidStateType.Physics or
               ...
               humState == Enum.HumanoidStateType.FallingDown then
                ...
                pcall(function()
                    ...
                    if pm then
                        local PlayerModule = pm:FindFirstChild("PlayerModule")
        ...
        table.insert(_antiRagdollConns, c1)

        -- Destruir constraints al instante
        local c2 = char.DescendantAdded:Connect(function(v)
            if not Config.AntiRagdollPrime then return end
            local isTping = State and State.isTpMoving
            if isTping then return end
        ...
        table.insert(_antiRagdollConns, c2)
    end

    local char = LocalPlayer.Character
    if char then task.spawn(function() setupChar(char) end) end

    local c3 = LocalPlayer.CharacterAdded:Connect(function(c)
        task.wait(0.3)
        task.spawn(function() setupChar(c) end)
    end)
    table.insert(_antiRagdollConns, c3)
end

local function stopAntiRagdollPrime()
    for _, c in pairs(_antiRagdollConns) do pcall(function() c:Disconnect() end) end
    _antiRagdollConns = {}
end

if Config.AntiRagdollPrime then task.spawn(function() task.wait(1); pcall(startAntiRagdollPrime) end) end
...
isOnCooldown = function(cmd)
    ...
    local removePlayer -- forward declaration para que el task.spawn dentro de createPlayerRow pueda verla
...
local function killSelf()
    ...
    -- Desactivar AntiDie temporalmente para poder morir
    local wasDisabled = _G.AntiDieDisabled
    ...
    -- Reactivar AntiDie despues de que el personaje respawnee
...
UserInputService.InputBegan:Connect(function(input, processed)
    ...
    if input.KeyCode == Enum.KeyCode.R then
        killSelf()
        ShowNotification("RESET", "Muriendo...")
    end
    
    if Config.RagdollSelfKey and Config.RagdollSelfKey ~= "" and input.KeyCode == Enum.KeyCode[Config.RagdollSelfKey] then
        task.spawn(function()
    ...
    if Config.DropBrainrotKey and Config.DropBrainrotKey ~= "" and input.KeyCode == (Enum.KeyCode[Config.DropBrainrotKey] or Enum.KeyCode.K) then
        task.spawn(doDropBrainrot)
        ShowNotification("DROP BRAINROT", "Activated!")
    end
...
if IS_MOBILE then
    ...
    local mobBtnIV = createMobBtn("IV", Color3.fromRGB(255, 50, 50), 4, function(self)
        if not LocalPlayer:GetAttribute("Stealing") and not _G.invisibleStealEnabled then
            ShowNotification("SEMI INVIS", "Solo funciona robando")
            return
        end
    ...
    -- Definir funcion global para actualizar el visual del boton IV
    _G.updateMovementPanelInvisVisual = function(isOn)
        if mobBtnIV and mobBtnIV.Parent then
            mobBtnIV.TextColor3 = isOn and Color3.fromRGB(255, 0, 0) or Theme.TextPrimary
        end
    end
...
local rArPrime = CreateRow("Anti-Ragdoll")
CreateToggleSwitch(rArPrime, Config.AntiRagdollPrime, function(ns, set)
    set(ns)
    Config.AntiRagdollPrime = ns
    SaveConfig()
    if ns then
        startAntiRagdollPrime()
    else
        stopAntiRagdollPrime()
    end
...
CreateToggleSwitch(rBaseTimerESP, Config.BaseTimerESP, function(ns, set)
    set(ns)
    if ns then enableBaseTimerESP() else disableBaseTimerESP() end
end)
...
do
    local rDropBrainrotKey = CreateRow("Drop Brainrot")
    local bDropBrainrotKey = Instance.new("TextButton", rDropBrainrotKey)
    bDropBrainrotKey.Size=UDim2.new(0,60,0,24); bDropBrainrotKey.Position=UDim2.new(1,-70,0.5,-12)
    bDropBrainrotKey.BackgroundColor3=Theme.SurfaceHighlight; bDropBrainrotKey.Text=Config.DropBrainrotKey or "K"
    bDropBrainrotKey.Font=Enum.Font.GothamBold; bDropBrainrotKey.TextColor3=Theme.TextPrimary; bDropBrainrotKey.TextSize=12
    Instance.new("UICorner",bDropBrainrotKey).CornerRadius=UDim.new(0,4)
    bDropBrainrotKey.MouseButton1Click:Connect(function()
        bDropBrainrotKey.Text="..."; bDropBrainrotKey.TextColor3=Theme.Accent1
        local con; con=UserInputService.InputBegan:Connect(function(inp)
            if inp.UserInputType==Enum.UserInputType.Keyboard then
                Config.DropBrainrotKey=inp.KeyCode.Name; bDropBrainrotKey.Text=inp.KeyCode.Name
                bDropBrainrotKey.TextColor3=Theme.TextPrimary; SaveConfig(); con:Disconnect()
                ShowNotification("DROP BRAINROT KEYBIND", inp.KeyCode.Name)
            end
        end)
    end)
end
...
local rLaserAimbot = CreateRow("Laser Cape Aimbot")
CreateToggleSwitch(rLaserAimbot, Config.LaserCapeAimbot, function(ns, set)
    set(ns); Config.LaserCapeAimbot = ns; SaveConfig()
    ShowNotification("LASER CAPE AIMBOT", ns and "ENABLED" or "DISABLED")
end)
...
if not IS_MOBILE then
    UserInputService.InputBegan:Connect(function(input, gp)
        ...
        if Config.ProximityAPKeybind and Config.ProximityAPKeybind ~= "" and Enum.KeyCode[Config.ProximityAPKeybind] and input.KeyCode == Enum.KeyCode[Config.ProximityAPKeybind] then
            ProximityAPActive = not ProximityAPActive
            if SharedState.ProximityAPButton then
                updateProximityAPButton()
            end
        ...
        if Config.ClickToAPKeybind and Config.ClickToAPKeybind ~= "" and input.KeyCode == (Enum.KeyCode[Config.ClickToAPKeybind] or Enum.KeyCode.L) then
            Config.ClickToAP = not Config.ClickToAP
            SaveConfig()
            ShowNotification("CLICK TO AP", Config.ClickToAP and "ENABLED" or "DISABLED")
        end
        if Config.JobJoinerKey and Config.JobJoinerKey ~= "" and Enum.KeyCode[Config.JobJoinerKey] and input.KeyCode == Enum.KeyCode[Config.JobJoinerKey] then
        ...
        if Config.DropBrainrotKey and Config.DropBrainrotKey ~= "" and Enum.KeyCode[Config.DropBrainrotKey] and input.KeyCode == Enum.KeyCode[Config.DropBrainrotKey] then
            task.spawn(doDropBrainrot)
            ShowNotification("DROP BRAINROT", "Activated!")
        end
    end)
...
task.spawn(function()
        ...
        local function doClone()
                ...
                if character and character:FindFirstChild("Humanoid") and character.Humanoid.Health > 0 then
                        ...
                        local _okGC, _gc = pcall(function() return oldRoot and oldRoot:GetChildren() or {} end)
                                for _, c in pairs(_okGC and _gc or {}) do
                                if c:IsA("Attachment") and (c.Name:find("Beam") or c.Name:find("Attach")) then c:Destroy() end
                        end
                        local _okGC2, _gc2 = pcall(function() return oldRoot and oldRoot:GetChildren() or {} end)
                        for _, c in pairs(_okGC2 and _gc2 or {}) do if c:IsA("Beam") then pcall(function() c:Destroy() end) end end
        ...
        local function revertClone()
                local character = LocalPlayer.Character
                -- Fix: allow revert even if oldRoot is in Camera (not just Workspace)
                if not oldRoot or not oldRoot.Parent or not character then
        ...
        local updateVisualState -- forward declaration
local function turnOff()
                ...
                if humanoid and humanoid.Health > 0 then pcall(function() humanoid:ChangeState(Enum.HumanoidStateType.GettingUp) end) end
                        ...
                        connection = RunService.PreSimulation:Connect(function()
                                if character and character:FindFirstChild("Humanoid") and character.Humanoid.Health > 0 and oldRoot then
                                        ...
                                        if root then
                                                        ...
                                                        if jumpDist > 3 and not _G.RecoveryInProgress then
                                                                ...
                                                                if _G.AutoRecoverLagback and _G.toggleInvisibleSteal then
                                                                        ...
                                                                        task.spawn(function()
                                                                                ...
                                                                                pcall(_G.toggleInvisibleSteal)
                                                                                task.wait(0.5)
                                                                                -- Solo re-encender si todavia esta robando y el invis sigue apagado
                                                                                local stillStealing = LocalPlayer:GetAttribute("Stealing")
                                                                                if stillStealing and not _G.invisibleStealEnabled and not animPlaying then
                                                                                        pcall(_G.toggleInvisibleSteal)
                                                                                end
                                                                                _G.RecoveryInProgress = false
                                                                        end)
                                                ...
                                                local _okGC3, _gc3 = pcall(function() return oldRoot and oldRoot:GetChildren() or {} end)
                                                        for _, c in pairs(_okGC3 and _gc3 or {}) do
    ...
    btnFix.MouseButton1Click:Connect(function()
        if not LocalPlayer:GetAttribute("Stealing") and not _G.AutoRecoverLagback then
            ShowNotification("AUTO FIX LAGBACK", "Solo funciona robando")
            return
        end
    ...
    btnInvis.MouseButton1Click:Connect(function()
        ...
        local function onCharacterAdded(newChar)
                ...
                if oldRoot then pcall(function() oldRoot:Destroy() end); oldRoot = nil end
                if clone then pcall(function() clone:Destroy() end); clone = nil end
                animPlaying = false; _G.invisibleStealEnabled = false; _G.RecoveryInProgress = false
                if _G.updateMovementPanelInvisVisual then pcall(_G.updateMovementPanelInvisVisual, false) end
                task.wait(0.2)
                local camera = Workspace.CurrentCamera
    ...
    task.spawn(function()
        ...
        local function setupAntiDie()
            ...
            -- 1. Prevenir muerte por Health <= 0
            ...
            -- 2. Heartbeat: vigilar Health sin tocar RagdollEndTime para evitar loops
            heartbeatConn = RunService.Heartbeat:Connect(function()
                ...
                -- Mantener vida al maximo si cae por debajo del 50%
                if hum.Health <= 0 then
                    hum.Health = hum.MaxHealth
                elseif hum.Health < hum.MaxHealth * 0.5 then
                    hum.Health = hum.MaxHealth
                end
            end)
        end
...
task.spawn(function()
    ...
    while task.wait(0.1) do
        if not Config.AutoInvisDuringSteal then
            ...
            if not isStealing and wasStealingForInvis then
                if autoEnabledInvis and _G.invisibleStealEnabled and _G.toggleInvisibleSteal then
                    task.delay(0.3, function()
                        if not LocalPlayer:GetAttribute("Stealing") and _G.invisibleStealEnabled and autoEnabledInvis then
                            if (tick() - lastToggleTime) >= TOGGLE_COOLDOWN then
                                pcall(_G.toggleInvisibleSteal)
                                lastToggleTime = tick()
                            end
                        end
...
task.spawn(function()
    ...
    -- ============================================================
    -- AUTO DESTROY TURRETS (multi-torreta en paralelo, con reintento
    -- si se destruye en cliente pero sigue en Workspace)
    -- ============================================================
    local MY_SENTRY_NAME = "Sentry_" .. tostring(LocalPlayer.UserId)
    ...
    local function getSentryModel(inst)
        if inst:IsA("Model") then return inst end
        return inst:FindFirstAncestorOfClass("Model") or inst
    end

    local function getAllSentries()
        local list = {}
        for _, inst in ipairs(Workspace:GetChildren()) do
            if inst.Name:match("^Sentry_") and inst.Name ~= MY_SENTRY_NAME then
    ...
    local function attackSentry(sentryInst)
        local sentryRoot = getSentryModel(sentryInst)
        local sentryName = sentryRoot.Name

        task.wait(1)
        if not sentryRoot or not sentryRoot.Parent then
            local stillExists = Workspace:FindFirstChild(sentryName)
            if stillExists then
                sentryRoot = getSentryModel(stillExists)
            else
                activeSentries[sentryName] = nil
                return
            end
        end
        ...
        repeat
            ...
            local cf = CFrame.new(hrp.Position + offset, hrp.Position)
            if sentryRoot:IsA("Model") then
                pcall(function() sentryRoot:PivotTo(cf) end)
            elseif sentryRoot:IsA("BasePart") then
                pcall(function() sentryRoot.CFrame = cf end)
            end
            applyVisuals(sentryRoot)

            while sentryRoot and sentryRoot.Parent and LocalPlayer:GetAttribute("Stealing") ~= true do
                local c2 = LocalPlayer.Character; if not c2 then break end
                local hum2 = c2:FindFirstChildOfClass("Humanoid")
                local bat2 = LocalPlayer.Backpack:FindFirstChild("Bat") or c2:FindFirstChild("Bat")
                if bat2 then
                    if bat2.Parent ~= c2 then hum2:EquipTool(bat2) end
                    bat2:Activate()
                end
                task.wait(0.1)
                if not hasExclamation(sentryRoot) then break end
            end

            if (not sentryRoot or not sentryRoot.Parent) and LocalPlayer:GetAttribute("Stealing") ~= true then
                local stillExists = Workspace:FindFirstChild(sentryName)
                if stillExists then
                    sentryRoot = getSentryModel(stillExists)
                    task.wait(0.2)
            ...
            elseif sentryRoot and sentryRoot.Parent then
                task.wait(0.3)
            end
        until not sentryRoot or not sentryRoot.Parent or LocalPlayer:GetAttribute("Stealing") == true
    ...
    while true do
        task.wait(0.5)
        if not Config.AutoDestroyTurrets then continue end
        if LocalPlayer:GetAttribute("Stealing") == true then task.wait(0.5); continue end

        local sentries = getAllSentries()
        for _, sentry in ipairs(sentries) do
            local name = getSentryModel(sentry).Name
            if not activeSentries[name] then
                activeSentries[name] = true
                task.spawn(attackSentry, sentry)
            end
        end
    end
end)
...
task.spawn(function()
            ...
            if p ~= LocalPlayer then
                ...
                if c then
                    ...
                    if h and hum and hum.Health > 0 then
                        ...
                        if d < best then best = d; closestHRP = h end
                    end
                end
            end
        end
    ...
    local function fireUseItemAtTarget(targetHRP)
        pcall(function()
            local remote = Utility:LarpNet("RE/UseItem")
            ...
            -- Proyectar la posicion del target a coordenadas de viewport
            local screenPos, onScreen = camera:WorldToViewportPoint(targetPos)

            local hitPosition, hitTarget

            if onScreen then
                -- Raycast desde el rayo de camara apuntando al target
                ...
                if result then
                    -- FIX: verificar si el raycast golpeo al personaje del target
                    -- Si hay una pared u obstaculo en el medio, usar posicion directa del HRP
                    local hitInst = result.Instance
                    local hitAncestor = hitInst:FindFirstAncestorOfClass("Model")
                    if hitAncestor and hitAncestor == targetChar then
                        -- Raycast llego al target correctamente
                        hitPosition = result.Position
                        hitTarget = hitInst
                    else
                        -- Hay una pared o roca entre la camara y el target
                        -- Apuntar directamente al HRP para ignorar obstaculos
                ...
                -- Target fuera de pantalla: mandar posicion directa
                hitPosition = targetPos
                hitTarget = targetHRP
            end

            -- FireServer igual que la capa: (Hit.Position, Target)
            remote:FireServer(hitPosition, hitTarget)
        end)
    end

    local hookedLaserCapes = {}
    local function hookLaserCape(tool)
        if hookedLaserCapes[tool] then return end
        hookedLaserCapes[tool] = true
        tool.Activated:Connect(function()
            if not Config.LaserCapeAimbot then return end
            local targetHRP = getNearestPlayerHRP()
            if targetHRP then fireUseItemAtTarget(targetHRP) end
        end)
        tool.AncestryChanged:Connect(function() hookedLaserCapes[tool] = nil end)
    end

    local function watchContainer(container)
        for _, t in ipairs(container:GetChildren()) do
            if t.Name == "Laser Cape" then hookLaserCape(t) end
        end
    ...
    LocalPlayer.CharacterAdded:Connect(function(char)
        -- Re-watchear el Backpack al respawnear (se vacia y repopulea)
        watchContainer(LocalPlayer.Backpack)
...
task.spawn(function()
    ...
    -- Logo a la izquierda del titulo (reemplaza la linea vertical)
    local logoHud = Instance.new("ImageLabel", topBar)
    logoHud.Size = UDim2.new(0, 30*SCALE, 0, 30*SCALE)
    logoHud.Position = UDim2.new(0, 8*SCALE, 0.5, 0)
    logoHud.AnchorPoint = Vector2.new(0, 0.5)
    logoHud.BackgroundTransparency = 1
    logoHud.ScaleType = Enum.ScaleType.Fit
    logoHud.ImageColor3 = Color3.fromRGB(255, 255, 255)
    logoHud.Image = "rbxassetid://110704100846087"

    local title = Instance.new("TextLabel", topBar)
    if privateBuild then
        title.Text = "SAHUR HUB PRIVATE"
    else
        title.Text = "SAHUR HUB"
    end
    title.Font = Enum.Font.GothamBlack
    ...
    title.Position = UDim2.new(0, 44*SCALE, 0.5, 0)
    ...
    if privateBuild then
        ...
        author.Text = "discord.gg/8HNEwrZUut"
    end
    ...
    author.Position = UDim2.new(0, 182*SCALE, 0.5, 0)
...
task.spawn(function()
    ...
    local function getPlayerColor(userId)
        if not playerColorMap[userId] then
            colorIndex = (colorIndex % #ESP_COLORS) + 1
            playerColorMap[userId] = ESP_COLORS[colorIndex]
        end
        return playerColorMap[userId]
    end

    local function makePlayerBillboard(player)
        local color = getPlayerColor(player.UserId)

        -- Highlight (borde de color en el personaje)
        local highlight = Instance.new("Highlight")
    ...
    local function createOrRefresh(player)
        ...
        if not entry or not entry.bb or not entry.bb.Parent then
            ...
            if entry and entry.highlight then pcall(function() entry.highlight:Destroy() end) end
            local bb, nameLbl, highlight = makePlayerBillboard(player)
            bb.Adornee = hrp; bb.Parent = hrp
            playerBillboards[uid] = {bb=bb, nameLbl=nameLbl, player=player, highlight=highlight}
        else
            if entry.bb.Adornee ~= hrp then entry.bb.Adornee = hrp; entry.bb.Parent = hrp end
            -- Re-attach highlight if character respawned
            if entry.highlight and player.Character and entry.highlight.Parent ~= player.Character then
                entry.highlight.Adornee = player.Character
                entry.highlight.Parent = player.Character
            end
        end
    end
